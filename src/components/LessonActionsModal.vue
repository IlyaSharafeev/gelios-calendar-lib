...
import ConfirmationModal from '../components/modals/ConfirmationModal.vue'; // Импортируем новый компонент
...
// Вычисляемые свойства для текста в модальном окне подтверждения
...
const resetModalState = () => {
  currentView.value = 'actions';
  editableLesson.value = { id: null, lessonDate: null, teacherId: null, teacherName: null };
};
...
const closeConfirmationModal = () => {
  ...
  reschedulePayload.value = null;
};

// --- Логика подтверждения ---

const triggerCancelLesson = () => {
  actionToConfirm.value = 'cancel';
...
const triggerReschedule = () => {
  ...
  actionToConfirm.value = 'reschedule';
  isConfirmModalOpen.value = true;
};

const handleConfirmation = () => {
  if (actionToConfirm.value === 'cancel') {
    emit('cancelLesson', props.lesson);
  } else if (actionToConfirm.value === 'reschedule' && reschedulePayload.value) {
    emit('rescheduleLesson', reschedulePayload.value);
  }
  closeConfirmationModal();
  close(); // Закрываем основное модальное окно
};
      ...
      <div class="modal-content" :class="{ 'animate-fade-in-up-custom': !isClosing }">
        ...
        <div v-if="isActionsView" class="action-buttons-container">
          <a v-if="(viewMode === 'teacher' || !viewMode) && props.lesson && props.lesson.link"
             :href="props.lesson.link" target="_blank" rel="noopener noreferrer"
             class="action-button primary-button" @click="close">
